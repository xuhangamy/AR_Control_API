...
namespace whu {
	namespace ardrone {
		...
		static inline uint32_t ardrone_set_state(uint32_t state, uint32_t mask, bool_t value) {
			state &= ~mask;
			if (value == true_t) {
				state |= mask;
			}
		...
		static inline bool_t ardrone_get_mask(uint32_t state, uint32_t mask) {
			return state&mask ? true_t : false_t;
		}
